...
export function PlanPricingForm({
  ...
  return (
    <ActionForm action={updatePlanPricingAction.bind(null, plan.id)} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1.4fr_1.4fr_1.4fr_1.4fr_auto] lg:items-end">
      {(pending) => (
        <>
          ...
          <TextField name="safepayPlanIdMonthly" label="Safepay plan (monthly)" placeholder="plan_… (optional)" defaultValue={plan.safepayPlanIdMonthly ?? ""} />
          <TextField name="safepayPlanIdAnnual" label="Safepay plan (annual)" placeholder="plan_… (optional)" defaultValue={plan.safepayPlanIdAnnual ?? ""} />
