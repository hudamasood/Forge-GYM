...
2. **Membership plans:** Safepay bills subscriptions against plans created in its dashboard (Subscriptions → Plans; currency USD, amount in cents, interval MONTH or YEAR, `billing_cycles` 0). Create a monthly and an annual plan for each FORGE plan and paste the `plan_…` ids into Admin → Memberships. A plan without its Safepay id cannot be bought online. Without admin access you can also set them on a production deploy: add `SAFEPAY_PLAN_IDS="gym-floor-access-monthly=plan_…,yoga-unlimited-annual=plan_…"` (plan slug + `-monthly`/`-annual`), redeploy, then remove the variable.
